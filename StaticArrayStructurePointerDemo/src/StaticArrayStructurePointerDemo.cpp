/*
 Fichier StaticArrayStructurePointerDemo.cpp
 Auteur : Mattens Jean-Michel
 Programme : StaticArrayStructurePointerDemo lit les fiches signialitiques des étudiants
 d'une classe et les stocke dans un tableau d'enregistrements.
 Puis il met le premier caractère du nom,le premier caractère
 du prénom, le caractère définissant le sexe et la chaine de
 caractères définissant la section en majuscules.
...
void LireClasse(TStudent aClasse[], int *pNbr);
void MajusculesClasse(TStudent aClasse[], int nbr);
void AffClasse(TStudent aClasse[], int n);
//---------------------------------------------------------------------------
int main(int argc, char* argv[]) {
	TStudent classe[NBR_ETU_MAX];
	int nbrEtu;
...
void AffClasse(TStudent aClasse[], int nbr) {
